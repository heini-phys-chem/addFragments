...
class TS_Stage(Stage):
	...
	def _do_check_input(self, folders, req):
		...
		return self._check_orca_input(lines, folders, '! MP2 6-311G(d) OptTS NoFrozenCore ExtremeSCF')
...
class NF_Stage(Stage):
	...
	def validate_e2_ts(logfile):
		...
		return check_neg_freqs(data.vibfreqs) and check_displacements_e2(data.atomcoords[-1], data.vibdisps[0])
	...
	def _do_check_input(self, folders, req):
		lines = open('%s/run.inp' % folders[-1], errors='replace').readlines()
		if not self._check_orca_input(lines, folders, '! MP2 6-311G(d) OptTS NoFrozenCore ExtremeSCF NumFreq') and not self._check_orca_input(lines, folders, '! MP2 6-311G(d) NoFrozenCore ExtremeSCF NumFreq'):
			return False
